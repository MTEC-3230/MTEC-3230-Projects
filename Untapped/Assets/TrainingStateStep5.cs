...
public class TrainingStateStep5 : TrainingState
{
    ...
    public void Pour(GameObject a, GameObject b)
    {
        if (_manager._currentStateIndex == this.index)
        {
            ...
            if (a.name == "Gin bottle" && _manager.SelectedGlass.Equals(b))
            {
                completedSuccesfully = true;
            }
            else
            {
                if (a.name != "Gin bottle")
                {
                    completedSuccesfully = false;
                    _manager.SetState(_manager.step4);
                    _manager.step4.step1 = "Try again. Pick a new Coupe Glass.";
    ...
    public TrainingStateStep5(TrainingManager manager)
    {
        _stateName = "Tutorial Step 5";
    }
