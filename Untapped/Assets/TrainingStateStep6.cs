...
public class TrainingStateStep6 : TrainingState
{
    ...
    public void Pour(GameObject a, GameObject b)
    {
        if (_manager._currentStateIndex == this.index)
        {
            ...
            if (a.name == "Orange Bottle" && _manager.SelectedGlass.Equals(b))
            {
                completedSuccesfully = true;
            }
            else
            {
                if (a.name != "Orange Bottle")
                {
    ...
    public TrainingStateStep6(TrainingManager manager)
    {
        _stateName = "Tutorial Step 6";
    }
